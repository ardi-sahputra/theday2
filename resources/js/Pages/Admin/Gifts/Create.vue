...
<template>
    <Head :title="t('gift.admin.create.title')" />
        ...
        <div class="max-w-3xl mx-auto space-y-5">
            ...
            <header>
                <h2 class="text-xl font-semibold tracking-tight flex items-center gap-2">
                    ...
                    {{ t('gift.admin.create.title') }}
                </h2>
                <p class="text-sm text-muted-foreground mt-0.5">
                    {{ t('gift.admin.create.subtitle') }}
                </p>
            </header>
            ...
            <form @submit.prevent="submit" class="space-y-5">
                ...
                <Card>
                    <CardContent class="p-5 space-y-4">
                        <div class="space-y-2">
                            <Label for="plan_id">{{ t('gift.admin.create.plan_label') }} <span class="text-destructive">*</span></Label>
                            <Select v-model="form.plan_id">
                                <SelectTrigger id="plan_id" class="w-full h-10" :class="{ 'border-destructive': form.errors.plan_id }">
                                    <SelectValue :placeholder="t('gift.admin.create.plan_placeholder')" />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem
                                        v-for="plan in plans"
                        ...
                        <fieldset class="space-y-2">
                            <legend class="text-sm font-medium leading-none">
                                {{ t('gift.admin.create.delivery_legend') }} <span class="text-destructive">*</span>
                            </legend>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2" role="radiogroup" :aria-label="t('gift.admin.create.delivery_aria')">
                                <label
                                ...
                                >
                                    ...
                                    <span class="flex-1 min-w-0">
                                        <span class="block text-sm font-semibold">{{ t('gift.admin.create.delivery_link_title') }}</span>
                                        <span class="block text-xs text-muted-foreground mt-0.5">{{ t('gift.admin.create.delivery_link_desc') }}</span>
                                    </span>
                                </label>
                                ...
                                >
                                    ...
                                    <span class="flex-1 min-w-0">
                                        <span class="block text-sm font-semibold">{{ t('gift.admin.create.delivery_email_title') }}</span>
                                        <span class="block text-xs text-muted-foreground mt-0.5">{{ t('gift.admin.create.delivery_email_desc') }}</span>
                                    </span>
                                </label>
                            </div>
                        ...
                        <div v-if="form.delivery_mode === 'email'" class="space-y-2">
                            <Label for="recipient_email">
                                {{ t('gift.admin.create.recipient_email_label') }} <span class="text-destructive">*</span>
                            </Label>
                            <Input
                                id="recipient_email"
                        ...
                        <div class="space-y-2">
                            <div class="flex items-center justify-between">
                                <Label for="message">
                                    {{ t('gift.admin.create.message_label') }} <span class="text-muted-foreground font-normal">{{ t('gift.admin.create.message_optional') }}</span>
                                </Label>
                                <span
                            ...
                            <textarea
                                ...
                                rows="3"
                                maxlength="280"
                                :placeholder="t('gift.admin.create.message_placeholder')"
                ...
                <Card>
                    <CardContent class="p-5 space-y-4">
                        <div>
                            <h3 class="text-sm font-semibold">{{ t('gift.admin.create.advanced_title') }}</h3>
                            <p class="text-xs text-muted-foreground mt-0.5">
                                {{ t('gift.admin.create.advanced_sub') }}
                            </p>
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                <Label for="duration_days">{{ t('gift.admin.create.duration_label') }}</Label>
                                <Input
                                    id="duration_days"
                            ...
                            <div class="space-y-2">
                                <Label for="custom_expires_at">{{ t('gift.admin.create.expires_label') }}</Label>
                                <Input
                                ...
                                <p v-else class="text-xs text-muted-foreground">{{ t('gift.admin.create.expires_hint') }}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <div class="flex items-center justify-end gap-2 pt-1">
                    <Button as-child variant="ghost" type="button" class="h-10">
                        <Link href="/admin/gifts">{{ t('gift.admin.create.cancel') }}</Link>
                    </Button>
                    <Button type="submit" :disabled="form.processing" class="h-10 min-w-[140px]">
                        ...
                        {{ form.processing ? t('gift.admin.create.processing') : t('gift.admin.create.submit') }}
                    </Button>
                </div>
            </form>
        </div>
