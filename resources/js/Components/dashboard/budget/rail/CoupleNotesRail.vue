...
<template>
  <div class="rounded-[16px]" style="background:#FBFCF9; border:1px solid #D8DFD2;">
    <div class="px-5 pt-4 pb-3">
      <h3 class="font-medium text-[20px] tracking-tight" style="color:#1F2A2E;">{{ t('dashboard.budget.rail.notes.title') }}</h3>
    ...
    <div class="px-5 pb-4 flex flex-col gap-2.5">
      <div v-for="n in notes" :key="n.id" class="rounded-[10px] p-2.5" style="background:#F6F8F3; border:1px solid #D8DFD2;">
        <div class="flex items-center gap-2 mb-1">
          <div class="w-[22px] h-[22px] rounded-full grid place-items-center text-[10px] font-bold" :style="{ background: n.is_mine ? '#C7D3BC' : '#D9B5B0', color:'#1F2A2E' }">{{ n.author_initial }}</div>
