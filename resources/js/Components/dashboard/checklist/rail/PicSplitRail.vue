...
<template>
  <div class="rounded-[16px]" style="background:#FBFCF9; border:1px solid #D8DFD2;">
    <div class="px-5 pt-4 pb-3">
      <h3 class="font-medium text-[20px] tracking-tight" style="color:#1F2A2E;">{{ t('dashboard.checklist.rail.pic.title') }}</h3>
      ...
      <div v-for="row in [
        ...
        <div class="flex justify-between items-center mb-1.5">
          <div class="flex items-center gap-2">
            <span class="w-6 h-6 rounded-full grid place-items-center text-[11px] font-bold" :style="{ background: row.c, color:'#1F2A2E' }">{{ row.k }}</span>
