# AI New Template Guide Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Produce `docs/AI-NEW-TEMPLATE-GUIDE.md` plus integration hooks (CLAUDE.md link, boilerplate comment) so AI agents can scaffold new wedding invitation templates without hallucinating fields, bypassing the composable, or shipping flat/static templates.

**Architecture:** Single markdown reference doc at `docs/AI-NEW-TEMPLATE-GUIDE.md` containing 6 sections (TL;DR, Lifecycle Walkthrough, Reference Manual, Animation Requirements, Anti-Halu Rules, Definition of Done Checklist). Linked from project CLAUDE.md and from a top-of-file comment in `_template-boilerplate.vue` for discoverability.

**Tech Stack:** Markdown only. No build step. Linked from existing Vue/PHP project (Laravel + Inertia + Vue 3).

**Source spec:** [`docs/superpowers/specs/2026-05-17-ai-new-template-guide-design.md`](../specs/2026-05-17-ai-new-template-guide-design.md). Each task below maps to a specific section of the spec.
...
|---|---|---|
| `docs/AI-NEW-TEMPLATE-GUIDE.md` | Create | Main reference doc (6 sections) |
| `CLAUDE.md` | Modify | Add 1-line reference so AI auto-loads guide for template tasks |
| `resources/js/Components/invitation/templates/_template-boilerplate.vue` | Modify | Add top-of-file comment pointing to the guide |

---

## Task 1: Create doc file with header + TL;DR

**Files:**
- Create: `docs/AI-NEW-TEMPLATE-GUIDE.md`

- [ ] **Step 1: Create the file with header + TL;DR section**
...
Run: `ls -lh docs/AI-NEW-TEMPLATE-GUIDE.md`
Expected: file size > 1KB.

- [ ] **Step 3: Commit**

```bash
git add docs/AI-NEW-TEMPLATE-GUIDE.md
git commit -m "docs(ai-guide): scaffold AI new template guide with TL;DR"
```

---

## Task 2: Append Section 2 — Lifecycle Walkthrough

**Files:**
- Modify: `docs/AI-NEW-TEMPLATE-GUIDE.md`

- [ ] **Step 1: Append Section 2 content**

Append this exact content to the end of `docs/AI-NEW-TEMPLATE-GUIDE.md`:
...
Run: `grep -c "^## Section\|^### Stage" docs/AI-NEW-TEMPLATE-GUIDE.md`
Expected: at least 8 (1 section header + 7 stages).

- [ ] **Step 3: Commit**

```bash
git add docs/AI-NEW-TEMPLATE-GUIDE.md
git commit -m "docs(ai-guide): add Section 2 — Lifecycle Walkthrough (7 stages)"
```

---

## Task 3: Append Section 3 — Reference Manual
...
Run: `grep -c "^### 3\." docs/AI-NEW-TEMPLATE-GUIDE.md`
Expected: 3 (3.1, 3.2, 3.3).

- [ ] **Step 3: Commit**

```bash
git add docs/AI-NEW-TEMPLATE-GUIDE.md
git commit -m "docs(ai-guide): add Section 3 — Reference Manual (composable + section catalog + config schema)"
```

---

## Task 4: Append Section 4 — Animation Requirements
...
Run: `grep -c "kenburns\|stagger\|reduced-motion" docs/AI-NEW-TEMPLATE-GUIDE.md`
Expected: at least 6.
...
```bash
git add docs/AI-NEW-TEMPLATE-GUIDE.md
git commit -m "docs(ai-guide): add Section 4 — Animation Requirements (MUST/SHOULD/Forbidden)"
```

---

## Task 5: Append Section 5 — Anti-Halu Rules
...
**Reason:** Section yang valid sudah definite di [Section Catalog](#32-section-catalog). User toggle dari customize wizard berdasarkan key itu — key custom ga bisa di-toggle dari UI.
...
Run: `grep -c "^### Rule " docs/AI-NEW-TEMPLATE-GUIDE.md`
Expected: 8.
...
```bash
git add docs/AI-NEW-TEMPLATE-GUIDE.md
git commit -m "docs(ai-guide): add Section 5 — Anti-Halu Rules (8 forbidden patterns)"
```

---

## Task 6: Append Section 6 — Definition of Done Checklist
...
Run: `grep -c "^### 6\." docs/AI-NEW-TEMPLATE-GUIDE.md`
Expected: 9 (6.1 through 6.9).

Run: `wc -l docs/AI-NEW-TEMPLATE-GUIDE.md`
Expected: line count between 600 and 900.

- [ ] **Step 3: Commit**

```bash
git add docs/AI-NEW-TEMPLATE-GUIDE.md
git commit -m "docs(ai-guide): add Section 6 — Definition of Done Checklist (9 verifiable sub-sections)"
```

---

## Task 7: Add reference to project CLAUDE.md
...
Determine where to append the reference. If the file has a clear "Instructions" or top-level section, append near the top. Otherwise, append a new section at the end.

- [ ] **Step 2: Append reference section**
...
Run: `grep "AI-NEW-TEMPLATE-GUIDE" CLAUDE.md`
Expected: 1 match.
...
```bash
git add CLAUDE.md
git commit -m "docs(claude): link AI-NEW-TEMPLATE-GUIDE for new-template tasks"
```

---

## Task 8: Add discoverability comment in boilerplate

**Files:**
- Modify: `resources/js/Components/invitation/templates/_template-boilerplate.vue`

- [ ] **Step 1: Read first 5 lines of boilerplate**

Run: `head -5 resources/js/Components/invitation/templates/_template-boilerplate.vue`

- [ ] **Step 2: Insert comment at top of file (before `<script setup>`)**

Add this exact line at the very top of the file (line 1):
...
Run: `head -2 resources/js/Components/invitation/templates/_template-boilerplate.vue`
Expected: line 1 contains "AI:" comment, line 2 is `<script setup>`.

- [ ] **Step 4: Commit**

```bash
git add resources/js/Components/invitation/templates/_template-boilerplate.vue
git commit -m "docs(boilerplate): point AI to AI-NEW-TEMPLATE-GUIDE before editing"
```
...
- [ ] **Step 1: Verify final doc structure**

Run:
```bash
grep -c "^## " docs/AI-NEW-TEMPLATE-GUIDE.md
```
Expected: 7 (TL;DR, Section 2, Section 3, Section 4, Section 5, Section 6, Resources).

Run:
```bash
wc -l docs/AI-NEW-TEMPLATE-GUIDE.md
```
Expected: 600-900 lines.

- [ ] **Step 2: Scan for placeholder strings**

Run:
```bash
grep -nE "TBD|TODO|FIXME|XXX|\\\$YOUR|<placeholder>" docs/AI-NEW-TEMPLATE-GUIDE.md
```
Expected: no matches (only legitimate code examples may contain `<Name>Template`, that's intentional).

- [ ] **Step 3: Verify all internal links are valid**

Run:
```bash
grep -oE "\(\.\./[a-zA-Z0-9/_.-]+\)" docs/AI-NEW-TEMPLATE-GUIDE.md | sort -u
```
For each path printed (without the leading `../`), verify it exists:
```bash
for p in $(grep -oE "\.\./[a-zA-Z0-9/_.-]+" docs/AI-NEW-TEMPLATE-GUIDE.md | sort -u); do
    rel="${p#../}"
    if [ ! -e "$rel" ]; then echo "MISSING: $rel"; fi
done
```
Expected: no "MISSING" output.

- [ ] **Step 4: Quick render test**

Open `docs/AI-NEW-TEMPLATE-GUIDE.md` in a markdown previewer (VSCode preview, or any md viewer):
- Verify all code blocks render properly (no broken fence syntax)
- Verify tables render (Section 3.1, Section 3.2)
- Verify all 7 top-level sections present in TOC

If everything renders: doc is good.

- [ ] **Step 5: Final commit (no-op if previous tasks already committed everything)**

```bash
git status
# Should show: clean tree (everything committed in tasks 1-8)
```

If anything is uncommitted from previous tasks (e.g., stray edits), commit now:

```bash
git add -A
git commit -m "docs(ai-guide): final cleanup"
```
...
After completing all 9 tasks, the engineer should verify the plan was followed:

1. **Spec coverage** — every section in the spec (`2026-05-17-ai-new-template-guide-design.md`) is implemented as a task:
   - Section 1 (TL;DR) → Task 1
   - Section 2 (Lifecycle) → Task 2
   - Section 3 (Reference) → Task 3
   - Section 4 (Animation) → Task 4
   - Section 5 (Anti-Halu) → Task 5
   - Section 6 (DoD) → Task 6
   - Integration → Tasks 7 & 8
   - Verification → Task 9

2. **Placeholder scan** — search final doc for `TBD`, `TODO`, `<placeholder>`. Only `<Name>Template`, `<slug>`, `<tpl>` allowed as intentional template variables in code examples.

3. **Internal consistency** — verify section anchors in cross-references match actual heading text. `#32-section-catalog` should resolve to "### 3.2 Section Catalog" heading.
